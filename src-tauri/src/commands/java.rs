...
pub async fn list_java(extra_dirs: Vec<String>) -> Result<Vec<JavaRuntime>, CommandError> {
    ...
    .await
    .map_err(|e| CommandError::launch("Не удалось просканировать систему на наличие Java")
        .with_details(e.to_string()))
}

#[tauri::command]
pub async fn probe_java(path: String) -> Result<Option<JavaRuntime>, CommandError> {
    tauri::async_runtime::spawn_blocking(move || {
...
fn probe_candidate(candidate: Candidate) -> Option<JavaRuntime> {
    ...
    let is_64bit = matches!(
        arch.as_str(),
        "amd64" | "x86_64" | "aarch64" | "arm64" | "ppc64" | "ppc64le" | "s390x" | "riscv64"
